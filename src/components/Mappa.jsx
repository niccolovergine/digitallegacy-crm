...
export function MappaSezione({ auth, eventi, sbGetMappaAttivi, sbGetMappaTicket, sbGetMappaIscrizioni, sfidaStart, sfidaEnd, showToast }) {
  ...
  function radiusOf(cnt) { return 4 + Math.sqrt(cnt/maxCnt) * 16; }
      ...
      {loading ? (
        ...
        <div style={{ display:"flex", gap:20, flexWrap:"wrap", justifyContent:"center" }}>
          <div style={{ position:"relative" }}>
            <svg viewBox={"0 0 "+MAPW+" "+MAPH} width={MAPW*0.72} height={MAPH*0.72} style={{ background:"#050912", borderRadius:12, border:"1px solid var(--border2)" }}>
              ...
              {attivo.attivi && datiAttivi.punti.map((p,i)=>{
                ...
                return <circle key={"a"+i} cx={x} cy={y} r={radiusOf(p.cnt)} fill={LAYERS.attivi.color} fillOpacity="0.45" stroke={LAYERS.attivi.color} strokeWidth="1.5"
                  onMouseEnter={()=>setHover({x,y,label:p.citta+": "+p.cnt+" attivi"})} onMouseLeave={()=>setHover(null)} style={{cursor:"pointer"}} />;
              })}
              {attivo.ticket && datiTicket.punti.map((p,i)=>{
                const [x,y]=project(p.coord);
                return <circle key={"t"+i} cx={x} cy={y} r={radiusOf(p.cnt)*0.7} fill={LAYERS.ticket.color} fillOpacity="0.55" stroke={LAYERS.ticket.color} strokeWidth="1.5"
                  onMouseEnter={()=>setHover({x,y,label:p.citta+": "+p.cnt+" ticket"})} onMouseLeave={()=>setHover(null)} style={{cursor:"pointer"}} />;
              })}
              {attivo.iscrizioni && datiIscrizioni.punti.map((p,i)=>{
                const [x,y]=project(p.coord);
                return <circle key={"i"+i} cx={x} cy={y} r={radiusOf(p.cnt)*0.5} fill={LAYERS.iscrizioni.color} fillOpacity="0.65" stroke={LAYERS.iscrizioni.color} strokeWidth="1.5"
                  onMouseEnter={()=>setHover({x,y,label:p.citta+": "+p.cnt+" iscritti"})} onMouseLeave={()=>setHover(null)} style={{cursor:"pointer"}} />;
              })}
              {hover && (
                <g>
                  <rect x={hover.x+8} y={hover.y-24} width={hover.label.length*6.2+14} height={22} rx={6} fill="#000000dd" />
                  <text x={hover.x+15} y={hover.y-9} fontSize="11" fill="#fff" fontWeight="700">{hover.label}</text>
                </g>
              )}
            </svg>
            <div style={{ fontSize:10, color:"var(--border2)", marginTop:6, textAlign:"center" }}>Mappa semplificata, non in scala geografica esatta — solo per capire dove concentrarsi</div>
          </div>

          {nonRiconosciuteTot.length>0 && (
            <div style={{ maxWidth:260, background:"var(--bg3)", border:"1px solid var(--border2)", borderRadius:12, padding:"1rem", fontSize:11 }}>
              ...
              <div style={{ color:"var(--border2)", lineHeight:1.7, maxHeight:280, overflowY:"auto" }}>
                {nonRiconosciuteTot.map((c,i)=><div key={i}>{c}</div>)}
              </div>
              <div style={{ marginTop:8, color:"var(--muted)" }}>Probabilmente scritte in modo diverso dal solito (refusi, sigle) — non compaiono sulla mappa.</div>
            </div>
          )}
        </div>
      )}
